...
Bevor wir zur Co-Occurence-Analyse kommen, gibt es auch eine Funktion, die es einem schnell und unkompliziert ermöglicht herauszufinden, in welchem Kontext ein Wort benutzt wird. Die hier gemeinte Funktion ist die `kwic()`Funktion aus dem quanteda Packet. Mit folgendem Code können wir beispielsweise herausfinden, in welchem Kontext das Wort hero vorkommt. In der Ausgabe werden jeweils die fünf Wörter vor und nach dem Wort hero angezeigt.
...
Nun zur Co-Occurence-Analyse. Hierfür müssen wir zunächst die dfm in eine Feature Co-occurrence Matrix (fcm) umwandeln. Das machen wir mit der Funktion `fcm()` aus dem quanteda Packet.
...
#@Valerie, Fehler erkannt aber noch nicht gelößt :D
...
daten_df_udpipe <- daten_df$Description %>%
  ...
  udpipe(object = paste0(getwd(), "/data/english-ewt-ud-2.5-191206.udpipe", sep = "")) %>% 
  select(doc_id, sentence_id, token_id, token, head_token_id, dep_rel) %>%
  head(5)
```

Wer sich nun fragt, was diese Abkürzungen bedeuten, kann über die Hilfefunktion die Dokumentation von `udpipe()`aufrufen. Unter der Überschrift Value sind Links hinterlegt um den output zu interpretieren. 
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE}
?udpipe()
```
