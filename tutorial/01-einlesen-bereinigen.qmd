...
**Ein Beispiel für Encoding-Probleme**: Schauen wir uns ein Beispiel an: deutsche Umlaute.

Was passiert wenn wir hier das Encoding ändern?
...
**Ein Beispiel für Datenbereinigung mit regulären Ausdrücken**:
...
**Ein Beispiel für Datenfilterung mit regulären Ausdrücken**:
...
-   Tokenisierung: `tokens()` (hier können wir auch Sonderzeichen, wie Satzzeichen, entfernen)
...
**Mehr Infos zur Entfernung von Stoppwörtern**:
Es gibt verschiedene Möglichkeiten, Stoppwörter zu entfernen. Am einfachsten ist dies mithilfe der in quanteda integrierten Stoppwortlisten möglich. Diese sind in mehreren Sprachen verfügbar, darunter auch Deutsch. Je nach Forschungsfrage können diese Stoppwortlisten angepasst werden, indem eigene Stppwörter definiert und dann entfernt werden. Es ist aber auch möglich eine eigene Stoppwortliste zu erstellen.
...
dfm
```
Wir sehen: Relative pruning kann unseren Datensatz noch einmal deutlich verkleinern (und damit z.B. Analysen beschleunigen).
...
Das Ergebnis können wir mit einer Word Cloud visualisieren. Hierfür nutzen wir die `textplot_wordcloud()`Funktion aus dem `quanteda.textplots`-Paket.

Dabei werden besonders "häufige" Features grösser dargestellt.
