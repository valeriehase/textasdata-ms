...
```
Um das Ergebnis von der Berechnung zu interpretieren ploten wir die Daten. Hierfür nutzen wir das Packet ggplot2. 
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE} 
...
Und dann das gleiche Spiel für K = 6.
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE}
# Zuerst erstellen wir eine leere Spalte in unserem Dataframe data
...
data <- data %>%
  ...
  mutate(Year_Start = gsub("[^0-9]", "", Year),
         ...
         Year_Start = replace(Year_Start,
                              ...
                              2010))
```
Dann nehmen wir die Variable `Year_Start` als unabhängige Variable in unser Modell auf:
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE, results='hide'}
...
Diesen Effekt können wir nun auch grafisch darstellen.
