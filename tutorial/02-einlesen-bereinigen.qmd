...
Im letzten Schritt des Preprocessings entfernen wir häufige und selten vorkommende features aus der dfm. Das geht mit der Funktion `dfm_trim()`aus dem quanteda package. 
...
Frage: Wollen wir hier dazwischen noch ein Kapitel "erste Auswertungen" machen? Also topfeatures() und kwic() etc.? Und dann danach eine ausführliche Übung? Und soll die Übung dann hier rein oder in die Folien? Ich hätte jetzt fast Folien gesagt... kann mir die Übung aber gerne so oder so überlegen:) 
...
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE}
daten_fcm <- daten_fcm %>% 
  fcm_select(pattern = c("america", "crime", "family"), #hier noch bessere Begriffe
             selection = "keep")
```
...
```
Diese Visualisierung gibt uns nun aber noch keine genauen Angaben dazu, wie oft ein feature mit einem anderen vorkommt. Um das herauszufinden, müssen wir die fcm mit der `convert()`Funktion aus dem quanteda package in einen data frame umwandeln. 
```{r echo = TRUE, message = FALSE, warning = FALSE, error = FALSE}
