...
Die folgende Übung fasst alles zusammen, was wir bisher gelernt haben: Preprocessing und N-Grams.

Bitte arbeite für die Übung mit dem Horoskop-Datensatz (Download der CSV-Datei entweder via der Webseite oder Einlesen via [dieser](https://raw.githubusercontent.com/valeriehase/textasdata-ms/main/data/data_horoscope.csv) URL)

1.  Lese den Datensatz ein und verschaffe dir einen Überblick über die Daten. Welche Variablen sind dort vorhanden?
...
horoscope_df <- read.csv(text = url) #mit read.csv() und nicht read.csv2(), weil Spalten mit , getrennt sind

#hab jetzt hier nochmal gefiltert, weil die dfm so groß wird
horoscope_df <- horoscope_df[1:1000,]

#Überblick über Datensatz verschaffen
str(horoscope_df)
View(horoscope_df)

horoscope_df$Horoscope %>%
  head() %>% 
  writeLines()

#Variable Date in ein Datum umwandeln
horoscope_df$Date <- as.Date(horoscope_df$Date, format = "%d-%m-%Y")
str(horoscope_df)
```
...
5.  Finde heraus welcher der Planeten am meisten in Zusammenhang mit Veränderungen und Neuanfängen genannt wird. #Notiz für uns: Co-Occurrence
